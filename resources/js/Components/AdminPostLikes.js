...
const AdminPostLikes = (props) => {
  const { post } = props;
  const [allPostLikes, setPostAllLikes] = useState([]);

  const getLikesForPost = () => {
    axios.get(route(`${props.modulePanel}.adminposts.showLikes`, props.post.id)).then((res) => {
      setPostAllLikes(res.data.likes);
    });
  };

  useEffect(() => {
    getLikesForPost(post);
  }, []);
    ...
    <>
      <Modal open={open} aria-labelledby="modal-modal-title" aria-describedby="modal-modal-description">
        <div className="d-flex w-full justify-center items-center flex fixed inset-0 z-50 outline-none focus:outline-none">
          <div className="w-full max-w-md overflow-hidden text-left align-middle transition-all transform shadow-xl rounded-2xl m-2">
            <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
              ...
              <div className="px-6 py-0 pb-4 h-96 bg-gray-100 overflow-y-auto">
                {allPostLikes.length > 0 &&
                  allPostLikes.map((likedBy, index) => (
                      ...
                      <div className="flex items-center">
                        {likedBy.type === 'admin' &&
                          <>
                            ...
                            <div className="ml-2">
                              <Link
                                href={route('admin.adminProfile', likedBy.id)}
...
const AdminTeacherPostLikes = (props) => {
  const { post } = props;
  const [allPostLikes, setPostAllLikes] = useState([]);
                        ...
                        {likedBy.type === 'admin' &&
                          <>
                            ...
                            <div className="ml-2">
                              { props.isFriday == true ?
                                <Link
                                  href={route('teacher.classes.adminProfile', likedBy.id)}
                                  className="ml-2 hover:text-gray-900 focus:text-gray-900"
                                >
                                  {likedBy.name}
                                </Link>:
                                <p className="ml-2">{likedBy.name}</p>
                              }
                            </div>
                          </>
                        }
...
export {AdminPostLikes, AdminTeacherPostLikes };
