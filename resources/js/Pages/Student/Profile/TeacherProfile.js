...
export default function TeacherProfile(props) {
  const fetchLikeData = (event) => {
    ApiService.post(route('teacherProfile.likes', [event.target.value, event.target.checked]))
      .then(() => {
  ...
  const openPostLikeModal = (post) => (
    <TeacherClassPostLikes post={post} closeModel={callbackModal} modulePanel="student" />
  );

  return (
    <Index header={'TeacherProfile'}>
            ...
            <div className="relative flex-row items-start md:grid-cols-3 lg:grid grid-rows-auto p-3 bg-gray-200 rounded-lg border-0">
              {props.teacher &&
                props.teacher.map((post, index) => (
                  <>
                    <div
                      className={"relative item-center m-2 bg-white rounded-xl shadow-lg md:max-w-xl " + (post.media.length ? "row-span-3" : "row-span-1")}
                      key={index}
                    >
                      ...
                      <div className="text-gray-500 text-sm mb-6 mt-3 mx-3 px-2">
