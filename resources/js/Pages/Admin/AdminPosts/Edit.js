...
export default function Edit(props) {
  ...
  var defaultValue = data.content && data.content.replace(regextag,'<span class = "mention" data-index="0" data-denotation-char="" data-id="" data-value="$1 ">#xFEFF;<span contenteditable="false">$1</span>&#xFEFF;</span>');
  ...
  const submit = (e) => {
    ...
    axios
      .post(route('admin.adminposts.update', props.post.id), formPayLoad)
      .then((response) => {
        setProcessing(false);
          swal.fire({
            title: 'Success !',
          ...
          setErrors([]);
          window.location.replace(route('admin.adminposts'));
          reset();
  ...
  const deletePostMedia = (e, post, media) => {
    ...
    swal.fire({
      ...
      if (chosenButton.isConfirmed) {
        axios.delete(route('admin.adminposts.deleteMedia', [post.id, media.id])).then((response) => {
          swal.fire({
              title: 'Success !',
              text: response.data,
              icon: 'success',
  ...
  useEffect(() => {
    const fetchData = () => {
      ApiService.post(route('admin.adminposts.tagSomeone'),{'class_id':`${props.post.classes_id}`}).then((res) => {
              ...
              <div className="rounded-t bg-white mb-0 px-4 py-4">
                <div className="text-left flex items-start md:items-center justify-between flex-col md:flex-row">
                  <div className="w-full md:w-auto mb-2 md:mb-0 flex lg:justify-center justify-center items-start md:items-center flex-col lg:flex-row">
                    <h6 className="text-black-600 text-lg font-semibold capitalize">Edit Admin Post</h6>
                  </div>
                ...
                <div className="flex flex-wrap space-y-4">
                  <div className="w-full lg:w-6/12 px-4 mt-4">
                    ...
                    <Select
                      ...
                      name="classes_id"
                      className="mt-1"
                      onChange={(e) => setData('classes_id', e.value)}
