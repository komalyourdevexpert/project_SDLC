...
export default function Create(props) {
  ...
  const submit = (e) => {
    ...
    axios
      .post(route('admin.customFlaggedWords.store'), data)
      .then((response) => {
        setProcessing(false);
          Swal.fire({
            title: 'Success !',
          ...
          setErrors([]);
          reset();
          document.getElementById('formAddCustomFlaggedWord').reset();
      })
      .catch((err) => {
        setProcessing(false);
    ...
    <Authenticated auth={props.auth} errors={props.errors} header={'Own flagged words list'}>
      <div className="flex flex-wrap justify-center">
        <div className="w-full xl:w-3/5">
          <form onSubmit={submit} id="formAddCustomFlaggedWord">
            <div className="relative flex flex-col min-w-0 break-words w-full mb-4 md:mb-6 shadow-lg rounded-lg bg-gray-50 border-0">
              <div className="rounded-t bg-white mb-0 px-4 py-4">
                <div className="text-left flex items-start md:items-center justify-between flex-col md:flex-row">
                  <div className="w-full md:w-auto mb-2 md:mb-0 flex lg:justify-center justify-center items-start md:items-center flex-col lg:flex-row">
                    <h6 className="text-black-600 text-lg font-semibold capitalize">Add New Custom Flagged Word</h6>
                  </div>
                ...
                <div className="flex flex-wrap space-y-4">
                  <div className="w-full px-4 mt-4">
                    ...
                    <textarea
                      name="content_word"
                      id="content_word"
                      defaultValue={data.content_word}
                      rows="4"
                    ...
                    {errors.content_word && <div className="text-sm text-red-600">{errors.content_word}</div>}
                  </div>
