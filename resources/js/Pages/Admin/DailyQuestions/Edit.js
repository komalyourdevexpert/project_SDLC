...
export default function Edit(props) {
  ...
  const fetchStudents = async (e) => {
    ...
    setClassId(e.value);
    return await axios.get(route('admin.dailyQuestions.getStudents', [`${e.value}`,`${level_id}`]))
      .then((res) => {
  ...
  const fetchQuestion = async (e) => {
    ...
    setLevelId(e.value);
    return await axios.get(route('admin.dailyQuestions.fetch',[`${e.value}`,`${class_id}`])).then((res) => {
  ...
  const submit = (e) => {
    ...
    axios
      .patch(route('admin.dailyQuestions.update', props.question.id), data)
      .then((response) => {
        setErrors([]);
