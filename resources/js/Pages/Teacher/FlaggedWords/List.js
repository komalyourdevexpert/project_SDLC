...
export default function List(props) {
  ...
  const popupModel = (params) => (
    <ViewModal
      ...
      fetchRoute={route('teacher.flaggedWords.history', params.row.id)}
      params={params}
    />
  );
  const fetchRows = () => {
    ...
    axios.get(route('teacher.flaggedWords.fetch', { page: page > 0 ? page + 1 : null })).then((response) => {
      setIsLoading(false);
  ...
  const allColumns = [
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Link
          className="text-sm text-blue-600 p-2 rounded hover:bg-blue-100 font-medium"
          href={route('teacher.students.show', params.row.word_used_by.id)}
        >
