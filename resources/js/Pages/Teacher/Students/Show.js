...
export default function Show(props) {
  ...
  const student_data = props.student_data;
  ...
  return (
    <Authenticated auth={props.auth} errors={props.errors} header={'Students'}>
      <div className="flex flex-wrap students-post">
        <div className="flex flex-wrap w-full">
          <div className="w-full xl:w-1/2 left-side">
              ...
              <div className="rounded-t bg-white mb-0 px-4 py-4">
                <div className="text-left flex items-start md:items-center justify-between flex-col md:flex-row">
                  ...
                  <div className="flex items-center space-x-2">
                    <Link
                      href={route('teacher.students.edit', [props.student_data.id])}
                ...
                <div className="mb-2 max-w-md mx-auto bg-white rounded-lg overflow-hidden shadow-lg">
                    ...
                    <div className="text-left sm:text-left sm:flex m-2">
                      {props.profilePicture ? (
                        <img
                          ...
                          alt={`${student_data.first_name} ${student_data.last_name}`}
                        />
                      ) : (
                        <div className="h-12 w-12 md:w-12 md:h-12 relative flex justify-center items-center rounded-full text-2xl text-white uppercase bg-yellow-500">
                          {student_data.first_name.charAt(0)}
                        </div>
                      )}
                      ...
                      <div className="ml-2 py-2">
                        <h3 className="flex font-bold text-xl">
                          ...
                          {student_data.is_private == 1 && (
                            <div className="text-green-600 ml-1 mb-1">
                              <LockOutlinedIcon />
                            </div>
                          )}
                          {student_data.is_private == 0 && (
                            <div className="text-red-600 ml-1">
                              <LockOpenOutlinedIcon />
                            </div>
                          )}
                        </h3>
                        <div className="text-gray-700 my-2">{student_data.email}</div>
                        <div className="inline-flex text-grey-dark sm:flex items-center">
                          {student_data.is_active == 1 && (
                            <div className="text-green-500">
                              <ToggleOnIcon /> Active
                            </div>
                          )}
                          {student_data.is_active == 0 && (
                  ...
                  <div className="px-4 py-4">
                    ...
                    <div className="flex flex-col sm:flex-row">
                      {student_data.level && <span className="text-blue-500">Level: {student_data.level.name}</span>}
                    </div>
                    <div className="mt-2 flex flex-col sm:flex-row">
                      <span className="text-blue-500">Class Name: {student_data.classes[0].name}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="w-full xl:w-1/2 pl-0 xl:pl-8">
            <PostsList posts={student} student={student_data} students={students} />
          </div>
        </div>
      </div>
    </Authenticated>
