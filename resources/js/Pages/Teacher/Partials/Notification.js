...
export default function Notification(props) {
  ...
  return (
    <Authenticated auth={props.auth} errors={props.errors} header={'Notification'}>
                    ...
                    >
                      <div className="text-sm font-normal">
                        {notify.type.includes('TagNotification') && (
                          <Link
                            href={
                              notify.data.post_type === 'student_post_notification'
                                ? route('teacher.posts.show', notify.data.id)
                                  : notify.data.post_type === 'teacher_post'
                                  ? route('teacher.classPost.show', notify.data.id)
                                  : notify.data.post_type === 'teacher_post_comment'
                                  ? route('teacher.classPost.show', notify.data.id)
                                  : notify.data.post_type === 'student_post'
                                  ? route('teacher.posts.show', notify.data.id)
                                  :
                                  notify.data.post_type === 'admin_post'
                                  ? route('teacher.adminposts.show', notify.data.id)
                                  : notify.data.post_type === 'teacher' || notify.data.post_type === 'admin'
                                  ? route('teacher.adminposts.show', notify.data.id)
                                : route('teacher.dailyQuestions.show', notify.data.id)
                            }
                          >
                            <span className="font-bold">{notify.data.name}</span> {notify.data.data}
                          </Link>
                        )}
                        {notify.type.includes('FlaggedWordNotification') && (
                          
                          <Link
                            href={
                              notify.data.post_type === 'teacherpost_flaggedword'
                                && route('teacher.flaggedWords')
                            }
                          >
                            <span className="font-bold">{notify.data.name}</span> {notify.data.data}
                          </Link>
                        ...
                        {notify.type.includes('RegistrationNotification') && (
                          
                          <Link
                            href={
                              notify.data.post_type === 'student_registration'
                  ...
                  <>
                    ...
                    >
                      ...
                      <Link
                        ...
                        href={route('teacher.destroy.notification', notify.id)}
                        method="post"
                      >
                        Mark as read
                      </Link>
