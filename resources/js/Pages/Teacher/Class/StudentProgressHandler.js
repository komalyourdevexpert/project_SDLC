...
export default function StudentProgressHandler(props) {
  ...
  const submitStudentProgressHandler = (e) => {
    ...
    axios
      .patch(route('teacher.classes.updateStudentProgressHandler', division.id), data)
      .then((response) => {
        setProcessing(false);
          Swal.fire({
            title: 'Success !',
            ...
            <div className="flex flex-wrap">
              {students.length > 0 &&
                students.map((student) => (
                  <div className="w-full lg:w-6/12 px-4 pt-6" key={student.id}>
                    <div className="text-blueGray-700 font-semibold mb-2">{`${student.first_name} ${student.last_name}`}</div>
