...
export default function List(props) {
  ...
  const pinPost = (e, post, isPinPost, classId) => {
    e.preventDefault();
    Swal.fire({
      title: 'Warning!',
      text:
        isPinPost == true
          ? 'Are you sure you want to pin this class post? It will remove other pinned posts'
          : 'Are you sure you want to unpin this class post?',
      icon: 'warning',
      showConfirmButton: true,
      confirmButtonText: isPinPost == true ? 'Yes! pin this post' : 'Yes! unpin this post',
      showDenyButton: true,
      denyButtonText: 'No! Cancel',
    }).then((chosenButton) => {
      if (chosenButton.isConfirmed) {
        axios.patch(route('teacher.classPosts.pinpost', [post, isPinPost, classId])).then((response) => {
          Swal.fire({
            title: 'Success !',
            text: response.data,
            icon: 'success',
  ...
  const filteredData = props.rows.filter((el) => {
    ...
    if(inputText === 'my posts'){
      if(el.teacher_id == props.auth.user.id){
        return el;
      }
            ...
            <div className="rounded-t bg-white mb-0 px-4 py-4">
              <div className="text-left flex items-start md:items-center justify-between flex-col md:flex-row">
                <div className="w-full md:w-auto mb-2 md:mb-0 flex lg:justify-center justify-center items-start md:items-center flex-col md:flex-row">
                  <h6 className="text-black-600 text-lg font-semibold capitalize">Teacher&apos;s posts</h6>
                ...
                <Link
                  href={route('teacher.classPosts.create')}
                  ...
                  <li className="profile mb-6" key={index}>
                    <div className={'relative item-center m-0 md:m-2 bg-white rounded-xl shadow-lg md:max-w-xl'}>
                      <div className="flex justify-between">
                        ...
                        <span className="items-end text-sm font-semibold leading-none">
                          {post.is_pinpost == 1 ? (
                            <Checkbox
                              value={props}
                              id="like"
                              icon={<PushPinOutlinedIcon />}
                              checkedIcon={<PushPinIcon className="fill-pink" />}
                              name="checkedH"
                              className="checked fill-pink"
                              checked={true}
                              onClick={(e) => pinPost(e, post.id, false, post.classes_id)}
                            />
                          ) : (
                            <Checkbox
                              value={props}
                              ...
                              className="checked"
                              checked={false}
                              onClick={(e) => pinPost(e, post.id, true, post.classes_id)}
                            />
                          )}
                        </span>
                      </div>
